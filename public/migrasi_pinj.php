...
if (isset($_POST['action']) && $_POST['action'] == 'migrate') {
    ...
    try {
        ...
        sendLog('info', "🚀 Memulai proses migrasi untuk kecamatan: $nama_kec (ID: $lokasi)");
        
        if (empty($lokasi)) {
            throw new Exception("Lokasi tidak valid!");
        }
        
        $table_lkm = "pinjaman_anggota_" . intval($lokasi);
        $table_koperasi = "pinjaman_anggota_" . intval($lokasi);
        ...
        while ($row = $result_lkm->fetch_assoc()) {
            ...
            try {
                ...
                if (!empty($row['jaminan'])) {
                    ...
                    if ($jaminan_data && is_array($jaminan_data)) {
                        ...
                        foreach ($jaminan_data as $key => $value) {
                            if ($key != 'nilai_jaminan' && !empty($value) && $value != '-') {
                                $ket_parts[] = $value;
                            }
                        }
                        $keterangan = !empty($ket_parts) ? implode(', ', $ket_parts) : '';
                ...
                $data_proposal = sprintf(
                    "%s#%s#%s#%s#%s#%s#%s",
                    isset($row['tgl_proposal']) ? $row['tgl_proposal'] : '',
                    isset($row['proposal']) ? $row['proposal'] : '0',
                    isset($row['jangka']) ? $row['jangka'] : '0',
                    isset($row['pros_jasa']) ? $row['pros_jasa'] : '0',
                    isset($row['jenis_jasa']) ? $row['jenis_jasa'] : '0',
                    isset($row['sistem_angsuran']) ? $row['sistem_angsuran'] : '0',
                    isset($row['sa_jasa']) ? $row['sa_jasa'] : '0'
                );
                
                // Konstruksi data_verifikasi (base)
                $data_verifikasi_base = sprintf(
                    "%s#%s#%s#%s#%s#%s#%s",
                    isset($row['tgl_verifikasi']) ? $row['tgl_verifikasi'] : '',
                    isset($row['verifikasi']) ? $row['verifikasi'] : '0',
                    isset($row['jangka']) ? $row['jangka'] : '0',
                    isset($row['pros_jasa']) ? $row['pros_jasa'] : '0',
                    isset($row['jenis_jasa']) ? $row['jenis_jasa'] : '0',
                    isset($row['sistem_angsuran']) ? $row['sistem_angsuran'] : '0',
                ...
                if (!empty($row['nama_barang']) && $row['nama_barang'] != 'undefined') {
                ...
                // data_verifikasi1, 2, 3
                $user_id = isset($row['user_id']) ? $row['user_id'] : '0';
                $data_verifikasi1 = $data_verifikasi_base . "##" . $user_id;
                $data_verifikasi2 = $data_verifikasi_base . "##" . $user_id;
                $data_verifikasi3 = $data_verifikasi_base . "##" . $user_id;
                
                // data_waiting
                $tgl_tunggu = !empty($row['tgl_tunggu']) ? $row['tgl_tunggu'] : (isset($row['tgl_verifikasi']) ? $row['tgl_verifikasi'] : '');
                $data_waiting = $tgl_tunggu . "#" . $user_id;
                ...
                // Handle tanggal
                $tgl_cair_val = (!empty($row['tgl_cair']) && $row['tgl_cair'] != '0000-00-00') ? $row['tgl_cair'] : NULL;
                $tgl_lunas_val = (!empty($row['tgl_lunas']) && $row['tgl_lunas'] != '0000-00-00') ? $row['tgl_lunas'] : NULL;
                
                // Konversi jenis_pinjaman: 'I' => 1, selainnya => 2
                ...
                $failed++;
                $error_msg = "ID $current_id: " . $e->getMessage();
                $errors[] = $error_msg;
...
<body>
    <div class="container">
        ...
        <div class="log-container" id="logContainer">
            <div style="color: #888; margin-bottom: 10px;">═══ LOG MIGRASI ═══</div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#migrateForm").on('submit', function(e) {
                ...
                // Reset UI
                $("#logContainer").addClass('active').html('<div style="color: #888; margin-bottom: 10px;">═══ LOG MIGRASI ═══</div>');
                ...
                // Start migration with AJAX and EventSource simulation
                $.ajax({
                    url: window.location.href,
                    method: 'POST',
                    data: {
                                ...
                                if (line.startsWith('data: ')) {
                                    try {
                                        ...
                                        if (data.type === 'complete') {
                                            ...
                                            $("#btnMigrate").prop('disabled', false).text('Mulai Migrasi');
