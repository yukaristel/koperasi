    ...
    <div>
        ...
        <div class="row">
            <div class="col-md-2 mb-2">
                ...
                <input type="hidden" id="nia" name="nia" class="form-control"
                    value="{{ $anggota->id ?? '' }}">
            ...
            <div class="col-md-10 mb-2">
                ...
                <input type="text" class="form-control" id="namadepan" name="namadepan"
                    value="{{ $anggota->namadepan ?? '' }}" {{ $disabled }}>
            ...
            <div class="col-md-6 mb-2">
                ...
                <input type="text" class="form-control" id="nama_panggilan" name="nama_panggilan"
                    value="{{ $anggota->nama_panggilan ?? '' }}" {{ $disabled }}>
                <small class="text-danger" id="msg_nama_panggilan"></small>
            </div>

            <div class="col-md-6 mb-2">
                <label class="form-label">Jenis Kelamin</label>
                <select class="form-control select2" id="jk" name="jk" {{ $disabled }}>
            ...
            <div class="col-md-6 mb-2 d-flex gap-2">
                <div class="flex-fill">
                    ...
                    <input type="text" class="form-control" id="tempat_lahir" name="tempat_lahir"
                        value="{{ $anggota->tempat_lahir ?? '' }}" {{ $disabled }}>
                    <small class="text-danger" id="msg_tempat_lahir"></small>
                </div>
                <div class="flex-fill">
                    <label class="form-label">Tanggal Lahir</label>
                    <input type="text" class="form-control date" id="tgl_lahir" name="tgl_lahir"
                        value="{{ $anggota->tgl_lahir ?? '' }}" {{ $disabled }}>
            ...
            <div class="col-md-6 mb-2">
                ...
                <select class="form-control select2" id="desa" name="desa" {{ $disabled }}>
                    ...
                    @foreach ($desa as $d)
                        <option value="{{ $d->kd_desa }}"
                            {{ ($anggota->desa ?? null) == $d->kd_desa ? 'selected' : '' }}>
            ...
            <div class="col-12 mb-2">
                ...
                <input type="text" class="form-control" id="domisili" name="domisili"
                    value="{{ $anggota->domisili ?? '' }}" {{ $disabled }}>
            ...
            <div class="col-md-6 mb-2 d-flex gap-2 flex-column">
                <div class="d-flex gap-2">
                                ...
                                <optgroup label="{{ $jk->nama_jk }}">
                                    @foreach ($jk->usaha as $usaha)
                                        <option value="{{ $usaha->id }}"
                                            {{ ($anggota->usaha ?? '') == $usaha->id ? 'selected' : '' }}>
                                            {{ $usaha->nama_usaha }}
                                        </option>
                                    @endforeach
                                </optgroup>
                    ...
                    <div class="flex-fill">
                        ...
                        <input type="text" class="form-control" id="keterangan_usaha" name="keterangan_usaha"
                            value="{{ $anggota->keterangan_usaha ?? '' }}" {{ $disabled }}>
                    </div>
                </div>
            ...
            <div class="col-md-6 mb-2">
                ...
                <input type="text" class="form-control" id="tempat_kerja" name="tempat_kerja"
                    value="{{ $anggota->tempat_kerja ?? '' }}" {{ $disabled }}>
                <small class="text-danger" id="msg_tempat_kerja"></small>
            </div>

            <div class="col-md-6 mb-2">
                <label class="form-label">Pendidikan Terakhir</label>
                <select class="form-control select2" id="pendidikan" name="pendidikan" {{ $disabled }}>
                    <option value="">-- Pilih Pendidikan --</option>
                    <option value="Tidak Menempuh Pendidikan"
                        {{ isset($anggota->pendidikan) && $anggota->pendidikan == 'Tidak Menempuh Pendidikan' ? 'selected' : '' }}>
                        Tidak Menempuh Pendidikan</option>
                    <option value="SD/Sederajat"
                        {{ isset($anggota->pendidikan) && $anggota->pendidikan == 'SD/Sederajat' ? 'selected' : '' }}>
                        SD/Sederajat</option>
                    <option value="SLTP/Sederajat"
                        {{ isset($anggota->pendidikan) && $anggota->pendidikan == 'SLTP/Sederajat' ? 'selected' : '' }}>
                        SLTP/Sederajat</option>
                    <option value="SLTA/Sederajat"
                        {{ isset($anggota->pendidikan) && $anggota->pendidikan == 'SLTA/Sederajat' ? 'selected' : '' }}>
                        SLTA/Sederajat</option>
                    <option value="Diploma/Sederajat"
                        {{ isset($anggota->pendidikan) && $anggota->pendidikan == 'Diploma/Sederajat' ? 'selected' : '' }}>
                        Diploma/Sederajat</option>
                    <option value="Sarjana/Sederajat"
                        {{ isset($anggota->pendidikan) && $anggota->pendidikan == 'Sarjana/Sederajat' ? 'selected' : '' }}>
                        Sarjana/Sederajat</option>
                    <option value="Magister/Sederajat"
                        {{ isset($anggota->pendidikan) && $anggota->pendidikan == 'Magister/Sederajat' ? 'selected' : '' }}>
                        Magister/Sederajat</option>
                    <option value="Doktor/Sederajat"
                        {{ isset($anggota->pendidikan) && $anggota->pendidikan == 'Doktor/Sederajat' ? 'selected' : '' }}>
                        Doktor/Sederajat</option>
                </select>
            ...
            <div class="col-md-6 mb-2">
                ...
                <select class="form-control select2" id="agama" name="agama" {{ $disabled }}>
                    ...
                    <option value="Konghucu" {{ ($anggota->agama ?? '') == 'Konghucu' ? 'selected' : '' }}>Konghucu
                    </option>
                    <option value="Kepercayaan" {{ ($anggota->agama ?? '') == 'Kepercayaan' ? 'selected' : '' }}>
            ...
            <div class="col-md-6 mb-2">
                ...
                <select class="form-control select2" id="status_pernikahan" name="status_pernikahan"
                    {{ $disabled }}>
                    ...
                    <option value="Menikah" {{ ($anggota->status_pernikahan ?? '') == 'Menikah' ? 'selected' : '' }}>
                        Menikah</option>
                    <option value="Duda" {{ ($anggota->status_pernikahan ?? '') == 'Duda' ? 'selected' : '' }}>Duda
                    </option>
                    <option value="Janda" {{ ($anggota->status_pernikahan ?? '') == 'Janda' ? 'selected' : '' }}>
                        Janda</option>
                </select>
            </div>
            ...
            <div class="col-md-6 mb-2">
                ...
                <input type="text" class="form-control" id="nama_ibu" name="nama_ibu"
                    value="{{ $anggota->nama_ibu ?? '' }}" {{ $disabled }}>
        ...
        <div class="row">
            <div class="col-md-6 mb-2">
                ...
                <input type="text" class="form-control" id="nik_penjamin" name="nik_penjamin" maxlength="16"
                    value="{{ $anggota->nik_penjamin ?? '' }}" {{ $disabled }}>
                <small class="text-danger" id="msg_nik_penjamin"></small>
            </div>

            <div class="col-md-6 mb-2">
                <label class="form-label">Hubungan</label>
                <select class="form-control select2" id="hubungan_penjamin" name="hubungan_penjamin"
                    {{ $disabled }}>
                    <option value="">-- Pilih Hubungan --</option>
                    <option value="Suami/Istri"
                        {{ isset($anggota->hubungan) && $anggota->hubungan == 'Suami/Istri' ? 'selected' : '' }}>
                        Suami/Istri</option>
                    <option value="Orang Tua"
                        {{ isset($anggota->hubungan) && $anggota->hubungan == 'Orang Tua' ? 'selected' : '' }}>Orang
                        Tua</option>
                    <option value="Anak"
                        {{ isset($anggota->hubungan) && $anggota->hubungan == 'Anak' ? 'selected' : '' }}>Anak</option>
                    <option value="Saudara Kandung"
                        {{ isset($anggota->hubungan) && $anggota->hubungan == 'Saudara Kandung' ? 'selected' : '' }}>
                        Saudara Kandung</option>
                    <option value="Keluarga Lain"
                        {{ isset($anggota->hubungan) && $anggota->hubungan == 'Keluarga Lain' ? 'selected' : '' }}>
                        Keluarga Lain</option>
                    <option value="Teman/Kolega"
                        {{ isset($anggota->hubungan) && $anggota->hubungan == 'Teman/Kolega' ? 'selected' : '' }}>
                        Teman/Kolega</option>
                    <option value="Lainnya"
                        {{ isset($anggota->hubungan) && $anggota->hubungan == 'Lainnya' ? 'selected' : '' }}>Lainnya
                    </option>
                </select>
            ...
            <div class="col-md-12 mb-2">
                ...
                <input type="text" class="form-control" id="nama_penjamin" name="nama_penjamin"
                    value="{{ $anggota->penjamin ?? '' }}" {{ $disabled }}>
